...
- (void)showAt:(NSInteger)index{
    if (oldIndex_ != index) {
        ...
        UIViewController *vc = [self.dataSource TFSlideView:self controllerAt:index];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(TFSlideView:didSwitchTo:)]) {
            [self.delegate TFSlideView:self didSwitchTo:index];
        }
    }
}
...
- (void)repositionForOffsetX:(CGFloat)offsetx{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TFSlideView:switchingFrom:to:percent:)]) {
        [self.delegate TFSlideView:self switchingFrom:oldIndex_ to:panToIndex_ percent:fabs(offsetx)/self.bounds.size.width];
    }
}
...
- (void)backToOldWithOffset:(CGFloat)offsetx{
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(TFSlideView:switchCanceled:)]) {
            [self.delegate TFSlideView:self switchCanceled:oldIndex_];
        }
    }];
}
        ...
        if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInTFSlideView:self] && panToIndex_ != oldIndex_) {
            if (fabs(offsetx) > kPanSwitchOffsetThreshold) {
                ...
                [UIView animateWithDuration:animatedTime animations:^{
                    ...
                    if (self.delegate && [self.delegate respondsToSelector:@selector(TFSlideView:didSwitchTo:)]) {
                        [self.delegate TFSlideView:self didSwitchTo:oldIndex_];
                    }
                }];
            }else{
